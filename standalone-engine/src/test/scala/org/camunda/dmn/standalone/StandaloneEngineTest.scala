...
class StandaloneEngineTest
  ...
  it should "fail to evaluate decision by not existing id" in {
    ...
    engine.evalDecisionById("discount", Map()) should be(
      Left(Failure("No decision found with id 'discount'")))
  }

  it should "fail to evaluate decision by not existing name" in {
    val engine = StandaloneEngine.fileSystemRepository(emptyRepository)

    engine.evalDecisionByName("Discount", Map()) should be(
      Left(Failure("No decision found with name 'Discount'")))
  }

  it should "report evaluation failure" in {
    val engine = StandaloneEngine.fileSystemRepository(repository)

    engine.evalDecisionById("discount", Map("orderSize" -> 7)) should be(
      Left(Failure(
        "failed to evaluate expression 'customer': no variable found for name 'customer'")))
  }
  ...
  it should "fail to remove not existing resource" in {
    ...
    engine.removeResource("discount.dmn") should be(
      Left(Failure("No decisions found for resource 'discount.dmn'")))
  }
